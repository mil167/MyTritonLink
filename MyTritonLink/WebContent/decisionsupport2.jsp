...
<title>Decision Support 2 Query</title>
</head>
<body>
	<b>Decision Support 2 Query Form</b>
	<ul>
		<li>
	...
	<table>
		<tr>
			...
			<td>
				...
				%>
				<%-- PRESENTATION CODE --%>
				<form name="getstudent" method="post" action="decisionsupport2.jsp">
				...
				<%
					...
					PreparedStatement aplus = conn.prepareStatement(
							"SELECT COUNT(grade) FROM public.pastcourses WHERE grade = 'A+' AND coursecode = ? AND professor = ?");
					...
					PreparedStatement bplus = conn.prepareStatement(
							"SELECT COUNT(grade) FROM public.pastcourses WHERE grade = 'B+' AND coursecode = ? AND professor = ?");
					...
					PreparedStatement cplus = conn.prepareStatement(
							"SELECT COUNT(grade) FROM public.pastcourses WHERE grade = 'C+' AND coursecode = ? AND professor = ?");
					...
					PreparedStatement d = conn.prepareStatement(
							"SELECT COUNT(grade) FROM public.pastcourses WHERE grade = 'D' AND coursecode = ? AND professor = ?");
					d.setString(1, course);
					d.setString(2, prof);
					ResultSet rsd = d.executeQuery();
					
					PreparedStatement f = conn.prepareStatement(
							"SELECT COUNT(grade) FROM public.pastcourses WHERE grade = 'F' AND coursecode = ? AND professor = ?");
				...
				<table>
					...
					%>
						<tr>
							<form action="decisionsupport2.jsp" method="get">
